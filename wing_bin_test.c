...
wing_bin_err_t wing_token_get(uint8_t token, int32_t payload_len, uint8_t* buffer, void* user)
{
	elog_w(TAG, "get token = %02x , pl len = %d", token, payload_len);
	return WING_BIN_INTF_OK;
}//wing_token_get

wing_bin_err_t wing_frame_done(wing_bin_decoded_frame_t* decoded, void* user)
{
	...
	return WING_BIN_INTF_OK;//always ok
...
int tcpLoop() {
	...
	if (recvLen > 0) {
		wing_bin_err_t werr = wing_bin_rx_data(&wing_decode, recvBuf, recvLen);
		if (werr != WING_BIN_INTF_OK) {
			elog_e(TAG, "wing_bin_rx_data error");
		}
	}
	else if (recvLen == 0) {
...
int main() {
	...
	memset(&wing_intf, 0x00, sizeof(wing_bin_socket_intf_t));
	wing_intf.frame_done = wing_frame_done;
	wing_intf.frame_get = wing_token_get;
